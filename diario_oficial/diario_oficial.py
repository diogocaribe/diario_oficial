...
import datetime
import util as u
from models import DiarioOficialBruto
...
data_inicial = datetime.date(2024, 1, 18)  # 2024, 3, 15 tem um caso especial
...
def coletar_dado_data(data_inicial, data_final):
    while data_inicial <= data_final:
        coleta_diario_oficial(data=data_inicial)
        data_inicial += datetime.timedelta(days=1)


def json_carga_banco():
    """_summary_"""
    json_ = {
        'diario_oficial_json': coleta_diario_oficial(data_inicial),
        'edicao': 12345,
        'data': data_inicial,
        'exist': True,
    }
